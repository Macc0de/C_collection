...
int main()
{
	int array[] = {1,-2,3,-4,5,1,-100,-20,7,8,4}; // {1,-2,3,-4,5,1,-100,-20,7,8,4};
	// {5,2,3,9,4,-4,0,-1};
	int i, j;
	/*for (i = 0; i < 8; i++)
	{
	...
	while(current)
	{
		...
		{
			// Добавление элем. из интервала в новый список из старого
			addToList(&Second, current->data);
			// Удаляем из первого списка элементы из интервала
			if(previous == NULL)
			{
				First = First->Next;
				current = current->Next;
